...
% 
% q and Q now can be vectors Mx4 dimension.  they are quaternions.
...
function thetad = calculate_misorientation_quaternion_hcp_rough(q, Q)
q = quatnormalize(q);
Q = quatnormalize(Q);
...
S = repmat(S,size(q,1),1);
delta = quatmultiply(quatconj(q),Q);       % using function from aero...toolbox
delta = kron(delta, ones(12,1));
Delta = quatmultiply(delta, S);
thetad = 2*acosd( Delta(:,1) );
thetad = reshape(thetad, 12, []);
