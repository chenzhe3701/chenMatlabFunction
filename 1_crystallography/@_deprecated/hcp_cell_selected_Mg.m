...
euler = [299.3, 14.36, 46.95];  % artifical orientation in Mg

ssColor = ss_color('Mg');
phi_sys = [-90 180 0];  % UM
% phi_sys = [90 180 0];   % MSU
phi_error = [0 0 0];
...
% add a twin operation
% [ss,ca] = define_SS_cart('Mg','Twin');
% ss = ss(:,:,19);  % ts#19
% MTwin = dcm_by_twin(ss,4);
% g = MTwin * g;
...
Vertex(13,:) = [0 0 0 1];
Vertex(14,:) = 0.5*(Vertex(1,:)+Vertex(2,:));
Vertex(15,:) = 0.5*(Vertex(2,:)+Vertex(3,:));
Vertex(16,:) = 0.5*(Vertex(3,:)+Vertex(4,:));
Vertex(17,:) = 0.5*(Vertex(4,:)+Vertex(5,:));
Vertex(18,:) = 0.5*(Vertex(5,:)+Vertex(6,:));
Vertex(19,:) = 0.5*(Vertex(6,:)+Vertex(1,:));
...
Slpsys(13) = struct('number',13,'slp_plane',[1 1 -2 2],'slp_dir',[-1 -1 2 3],'n_vertex',4,'plane_index',[1 3 10 12],'burgers_index',[3 10]);
Slpsys(14) = struct('number',14,'slp_plane',[-1 2 -1 2],'slp_dir',[1 -2 1 3],'n_vertex',4,'plane_index',[2 4 11 7],'burgers_index',[4 11]);
Slpsys(15) = struct('number',15,'slp_plane',[-2 1 1 2],'slp_dir',[2 -1 -1 3],'n_vertex',4,'plane_index',[3 5 12 8],'burgers_index',[5 12]);
Slpsys(16) = struct('number',16,'slp_plane',[-1 -1 2 2],'slp_dir',[1 1 -2 3],'n_vertex',4,'plane_index',[4 6 7 9],'burgers_index',[6 7]);
Slpsys(17) = struct('number',17,'slp_plane',[1 -2 1 2],'slp_dir',[-1 2 -1 3],'n_vertex',4,'plane_index',[5 1 8 10],'burgers_index',[1 8]);
Slpsys(18) = struct('number',18,'slp_plane',[2 -1 -1 2],'slp_dir',[-2 1 1 3],'n_vertex',4,'plane_index',[6 2 9 11],'burgers_index',[2 9]);

% extension twin 
Slpsys(19) = struct('number',1,'slp_plane',[1 0 -1 2],'slp_dir',[-1 0 1 1],'n_vertex',4,'plane_index',[1 2 10 11],'burgers_index',[1 11]);
Slpsys(20) = struct('number',2,'slp_plane',[0 1 -1 2],'slp_dir',[0 -1 1 1],'n_vertex',4,'plane_index',[2 3 11 12],'burgers_index',[2 12]);
Slpsys(21) = struct('number',3,'slp_plane',[-1 1 0 2],'slp_dir',[1 -1 0 1],'n_vertex',4,'plane_index',[3 4 12 7],'burgers_index',[3 7]);
Slpsys(22) = struct('number',4,'slp_plane',[-1 0 1 2],'slp_dir',[1 0 -1 1],'n_vertex',4,'plane_index',[4 5 7 8],'burgers_index',[4 8]);
Slpsys(23) = struct('number',5,'slp_plane',[0 -1 1 2],'slp_dir',[0 1 -1 1],'n_vertex',4,'plane_index',[5 6 8 9],'burgers_index',[5 9]);
Slpsys(24) = struct('number',6,'slp_plane',[1 -1 0 2],'slp_dir',[-1 1 0 1],'n_vertex',4,'plane_index',[6 1 9 10],'burgers_index',[6 10]);
...
figure;
PLOT = 0;
plotPool = [23];     % -------- this is where you select what to plot ---------------------------------------------------- ---
for ii = 1:nss  % select %%%%%%%%%%%%%%%%%%%%%%%%%%%-2
    ...
    if ismember(ssn,plotPool)
        ...
        % (1) The following plot slip plane, this is where you select the plane, and color ----------------------------------------------------- 
%         if ssn==0
        if ismember(ssn,plotPool)&PLOT
            % Fill plane
            fill3(spx(1:slpsys(ssn).n_vertex),spy(1:slpsys(ssn).n_vertex),spz(1:slpsys(ssn).n_vertex),ssColor(ssn,:),'facealpha',0.6);
        ...
        if PLOT
        plot([slpsys(ssn).Trace(1),-slpsys(ssn).Trace(1)],[slpsys(ssn).Trace(2),-slpsys(ssn).Trace(2)],'--','Linewidth',2,'Color',ssColor(ssn,:));  % orange for pyramidal <c+a>
        end
%         if ssn>18
%             plot([slpsys(ssn).Trace(1),-slpsys(ssn).Trace(1)],[slpsys(ssn).Trace(2),-slpsys(ssn).Trace(2)],'--','Linewidth',2,'Color',[1 0.6 0])  % orange for pyramidal <c+a>
%         elseif ssn>12
