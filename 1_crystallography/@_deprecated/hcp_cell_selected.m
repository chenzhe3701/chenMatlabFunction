...
IMAGING_CONVENTION = 1;     % 1: origin at upper left. 0: origin at lower left
SETTING = 0;
euler = [358 170 92];
 
switch SETTING
    case 0
        phi_sys = [0 0 0];   % in same reference frame
    case 1
        phi_sys = [90 180 0];   % MSU
    case 2
        phi_sys = [-90 180 0];  % UCSB, UM(but UM should be setting 1)
    case 3
        phi_sys = [0 180 0];
    case 4
        phi_sys = [180 180 0];
    otherwise
        phi_sys = [-90 180 0];
        warndlg('SETTING = undefined , use custom phi_sys');         
end
