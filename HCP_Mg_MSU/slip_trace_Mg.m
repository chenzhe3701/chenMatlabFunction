...
% default c_a=1.62;  % c/a for Mg

% Zhe Chen, 2015-09-02 revised.
...
function [] = slip_trace_Mg(varargin)
...
if TF
    ...
    %  2nd order pyramidal <c+a>-glide
    ssa(:,:,7) = [1 1 -2 2; -1 -1 2 3];
    ssa(:,:,8) = [-1 2 -1 2; 1 -2 1 3];
    ssa(:,:,9) = [-2 1 1 2; 2 -1 -1 3];
    ssa(:,:,10) = [-1 -1 2 2; 1 1 -2 3];
    ssa(:,:,11) = [1 -2 1 2; -1 2 -1 3];
    ssa(:,:,12) = [2 -1 -1 2; -2 1 1 3];
    
    %  1st order pyramidal<a>-glide
    ssa(:,:,13) = [0 1 -1 1; 2 -1 -1 0];
    ssa(:,:,14) = [1 0 -1 1; 1 -2 1 0];
    ssa(:,:,15) = [-1 1 0 1; 1 1 -2 0];
    ssa(:,:,16) = [0 -1 1 1; -2 1 1 0];
    ssa(:,:,17) = [-1 0 1 1; -1 2 -1 0];
    ssa(:,:,18) = [1 -1 0 1; 1 1 -2 0];
    ...
    for i = 1:1:18            % Change n & m to unit vector
    ...
    for j = 1:1:18
        ...
        abs_schmid_factor(j,1) = abs(schmid_factor(j,1));  % used to evaluate rank of SF
    ...
    Sorted_SF = sortrows (abs_schmid_factor, -1);   % sort Schmid Factor
    
    for k = 1:1:18      % Plot traces
