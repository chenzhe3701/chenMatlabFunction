...
function [] = hcp_cell_Mg(varargin)

switch length(varargin)
    case 1
        Euler(:,3:5) = [varargin{1}(1),varargin{1}(2),varargin{1}(3)];
    case 3
        Euler(:,3:5) = [varargin{1}(1),varargin{2}(1),varargin{3}(1)];
    otherwise
        Euler(:,3:5) = [180   30  20];
end
display(['Euler angle in degree: ',num2str(Euler(3)),' ', num2str(Euler(4)), ' ', num2str(Euler(5))]);
phi_sys = -90;  % angle between EBSD data X axis and common-X-point-to-right X axis
phi_error = 0;  % angle error caused by placing the sample when doing experiment. Rotate EBSD into In-situ SEM orientation.
...
for set_number = 1:1:set_of_Euler(1,1)
    ...
    % for i = 1:1:nss %%%%%%%%%%%%%%%%%%%%%%%%%%%-1
    ii = 1;     % [For this code purpose] just plot one slip system
    ...
    ii = 1;     % [For this code purpose, only plot one slip system]
