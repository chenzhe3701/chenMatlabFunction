...
% chenzhe edit, based on function boxplot.
% Modification: if set 'whisker' to [a,b], then the whisker show a% and b%
% percentile, and the outliers are not plotted (but the default yLim
% reflects how large the outliers can be, and you may need to manually
% adjust the ylim).
%
...
% [chen edit] new line
if length(whisker) == 2
   for ii = 1:size(boxVal,1)
       boxVal.outliers{ii} = [];
       boxVal.numFiniteLoOutliers(ii) = 0;
       boxVal.numFiniteHiOutliers(ii) = 0;
       boxVal.outlierrows{ii} = [];
       
       boxValPlot.outliers{ii} = [];
       boxValPlot.numFiniteLoOutliers(ii) = 0;
       boxValPlot.numFiniteHiOutliers(ii) = 0;
       boxValPlot.outlierrows{ii} = [];
       
       boxIdx.outliers{ii} = [];
       boxIdx.numLoOutliers(ii) = 0;
       boxIdx.numHiOutliers(ii) = 0; 
   end
end
% [chen edit] new line
...
x = inargs{1};
inargs(1) = [];     % chenzhe, took x, then eliminate x from inargs
...
% end
if ((~isscalar(whisker))&&(length(whisker)~=2)) || ~isnumeric(whisker)   % chenzhe allowed whisker to be vector [low, high]
    error(message('stats:boxplot:BadWhisker'));
end
    ...
    if ischar(in{1}) && size(in{1},1)<=1 && isvector(in)
        ...
        for i=2:length(in)
            if ~ischar(in{i}) || size(in{1},1)>1
                display(['check line ',num2str(i),' in data']);
...
% [chen edit] if whisker == scalar, then it's the original code. If whisker == [low, high], then do my new code.
...
elseif length(whisker)==2
    if lastind > 1
        iLow = firstind+ whisker(1)*(lastind-firstind) - whisker(1);        % [chen edit] new line
        iHigh = firstind+ whisker(end)*(lastind-firstind) + whisker(end);   % [chen edit] new line
    else
        iHigh = 1;      % [chen edit] new line
        iLow = 1;       % [chen edit] new line
    end    
    
    pLowRatio = iLow-floor(iLow);
    if pLowRatio==0
        pLow = xSorted1group(iLow);
    else
        pLow = xSorted1group(floor(iLow))*(1-pLowRatio) ...
            +xSorted1group(ceil(iLow))*pLowRatio;
    end
    pHighRatio = iHigh-floor(iHigh);
    if pHighRatio==0
        pHigh = xSorted1group(iHigh);
    else
        pHigh = xSorted1group(floor(iHigh))*(1-pHighRatio) ...
            +xSorted1group(ceil(iHigh))*pHighRatio;
    end
