% chenzhe, 2018-02-13
% This is a hack of the matlab built-in function trainNetwork.
% This does not do any training, rather, it just initialize the network.

function [trainedNet, info] = trainNetwork_init(lgraph,varargin)
...
try
%     [lgraph, opts, X, Y] = iParseInputArguments(varargin{:});
    haveDAGNetwork = iHaveDAGNetwork(lgraph);
    if haveDAGNetwork
%         [trainedNet, info] = doTrainDAGNetwork(lgraph, opts, X, Y);
        [trainedNet] = doTrainDAGNetwork(lgraph);
    else
%         [trainedNet, info] = doTrainNetwork(lgraph, opts, X, Y);
        [trainedNet] = doTrainNetwork(lgraph);
    end
catch e
...
% function [trainedNet, info] = doTrainDAGNetwork(lgraph, opts, X, Y)
function [trainedNet] = doTrainDAGNetwork(lgraph)
haveDAGNetwork = true;
...
% function [trainedNet, info] = doTrainNetwork(layers, opts, X, Y)
function [trainedNet, info] = doTrainNetwork(layers)
