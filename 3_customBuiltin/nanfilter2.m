...
% Chenzhe 2016-5-17
%
% Based on Matlab function filter2.
% The diffrence is that this filter operates only on valid points, so that
% points with NaN values, or outside of boundareis don't contribute.
%
...
if isvector(stencil) || numel(stencil) > numel(x2)
    % The filter is bigger than the input.  This is a nontypical
    % case, and it may be counterproductive to check the
    % separability of the stencil.
    y2 = conv2(x2,stencil,shape);
else
    separable = false;
    % Stencil is considered to be not separable if the following test fails
    % for any reason, like non-finite stencil, empty stencil, etc.
    ...
    if separable
        ...
        hcol = u(:,1) * sqrt(s(1));
        hrow = conj(v(:,1)) * sqrt(s(1));
        y2 = conv2(hcol, hrow, x2, shape);
        if all(round(stencil(:)) == stencil(:)) && all(round(x2(:)) == x2(:))
            % Output should be integer
            y2 = round(y2);
        end
    else
