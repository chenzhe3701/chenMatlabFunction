...
function IPFMap = generate_IPF_map(ID,gID,gPhi1,gPhi,gPhi2,direction)
    R = ones(size(ID));
    G = ones(size(ID));
    B = ones(size(ID));
    for iGrain = 1:length(gID)
        currentID = gID(iGrain);
        RGB = calculate_IPF_color_hcp([gPhi1(iGrain),gPhi(iGrain),gPhi2(iGrain)],[direction(1),direction(2),direction(3)]);
        index = (ID==currentID);
        R(index) = RGB(1);
        G(index) = RGB(2);
        B(index) = RGB(3);
