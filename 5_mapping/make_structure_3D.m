% structure3D = make_structure_3D(gID3D,gID,gPhi1,gPhi,gPhi2,gNNeighbors,gNeighbors,gEdge3D,gDiameter3D,gVolume3D,boundaryID,neighborID,layerDepth)
%
% construct a structure which contains 3D info related to neighbors.
% structure3D(ii).ID: its grain ID
% .nNeighbor: number of neighbors
% .nieghbor: neighbors' IDs
% .misorientationArray{}: misorientation to each neighbor in different layers
% .misorientation: misorientation to neighbors
% .gbLengthArray{}: common gb length to each neighbor in different layers
% .gbArea: common gb Area to neighbors
% .neighborDiameter: diameter of neighbors
% .neighborVolume: volume of neighbors
% .gbAngleArray: empty, can hold angle of gb to neighbors
%
% Zhe Chen, 2015-08-10 revised.

function structure3D = make_structure_3D(gID3D,gID,gPhi1,gPhi,gPhi2,gNNeighbors,gNeighbors,gEdge3D,gDiameter3D,gVolume3D,boundaryID,neighborID,layerDepth)
...
display('constructed structure3D');
display(datestr(now));
