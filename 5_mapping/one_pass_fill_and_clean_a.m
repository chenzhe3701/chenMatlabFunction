...
% number to '0'
%
% chenzhe, 2019-01-20, try not to clean some small but long subclusters,
% but currently not working well.

function label = one_pass_fill_and_clean_a(raw, size_pct)
%%
...
% (a) new/subclusters corresponding to NaN values are definitely non-valid
label(isnan(raw)) = -1; 
labelNumToClean = [-1];   % change NaN in label to -1, and record it as toClean

uniqueLabel = unique(label(:));     
if any(isnan(uniqueLabel))
    disp('uniqueLabel has NaN, which should not happen');    % should not happen, but double check
    uniqueLabel(isnan(uniqueLabel)) = [];
end
stats = regionprops(label,'Eccentricity');
...
% (b) Continue to find out subclusters(labels) that need to be cleaned
for ii = 1:length(uniqueLabel)
   currentLabel = uniqueLabel(ii);
   % If subclusters that are too small and too round, clean it
   % else, if sublucster is too close to the grain boundary, clean it
   if (currentLabel>0)&&(gSize(ii)<min_size)&&(stats(currentLabel).Eccentricity<0.9)
       labelNumToClean = [labelNumToClean;currentLabel];
   else
       ind = label==currentLabel;
       ds = distMapLocal(ind);
...
useRaw = true(size(label));
useRaw(ismember(label,labelNumToClean)) = 0;
label(useRaw) = raw(useRaw);    % update label for the valid positions

nonValidLabel = unique(label(~useRaw(:)));
validLabel = unique(label(useRaw(:)==1));
